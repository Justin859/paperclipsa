...
<div class="container">
    <div class="row">
        ...
        @if($admin_notifications->count())
            @foreach($admin_notifications as $key=>$notification)
            ...
            ?>
                <li class="list-group-item list-item-notification">
                    ...
                    &nbsp;&nbsp;{{$user->firstname}} {{$user->surname}} requested to be an admin for <strong>{{$club->name}}</strong>
                    @if($notification->message)
                    <p style="color:#FFCC00">{{$notification->message}}</p>
                    @endif
                    <ul class="list-inline float-right m-0 p-0">
                        <li class="list-inline-item">
                            <form action="/user-profile/admin/notifications/request" method="post" class="m-0 p-0" name="accept_{{$key}}">
                            @csrf
                        ...
                        <li class="list-inline-item">
                            <form action="/user-profile/admin/notifications/request" method="post" class="m-0 p-0" name="decline_{{$key}}">
                            @csrf
